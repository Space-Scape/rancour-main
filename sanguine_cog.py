...
def matchmaking_algorithm(available_raiders: List[Dict[str, Any]]):
    ...
    for lb in learner_blocks:
        ...
        for t in teams:
            ...
            # STRICT LIMIT: Max 2 learners per mentor team
            current_learners = sum(1 for m in t['members'] if m['proficiency'] in ['learner', 'new'])
    ...
    while changed:
        ...
        def is_too_small(t):
            has_learner = any(p['proficiency'] in ['learner', 'new'] for p in t['members'])
            return len(t['members']) < 4 if has_learner else len(t['members']) < 3

        small_teams = sorted([t for t in teams if is_too_small(t)], key=lambda t: len(t['members']))
        ...
        large_teams = sorted([t for t in teams if can_spare(t)], key=lambda t: len(t['members']), reverse=True)
        ...
        for lt in large_teams:
            ...
            for p in sorted_candidates:
                ...
                has_learner_in_lt = any(m['proficiency'] in ['learner', 'new'] for m in lt['members'])
                if has_learner_in_lt:
    ...
    for t in teams:
        has_learner = any(p['proficiency'] in ['learner', 'new'] for p in t['members'])
        if has_learner and len(t['members']) < 4:
            stranded.extend(t['members'])
        elif len(t['members']) < 3 or len(t['members']) > 5:
            stranded.extend(t['members'])
        else:
            final_teams.append(t['members'])

    print(f"✅ Result: {len(final_teams)} Valid Teams, {len(stranded)} Stranded.")
...
def format_player_line_plain(guild: discord.Guild, p: dict) -> str:
    ...
    return f"{nickname} {region} • **{role_text}** {kc_text}{scythe}{freeze}"
...
def format_player_line_mention(guild: discord.Guild, p: dict) -> str:
    ...
    freeze = freeze_icon(p)
    region = f"[{p.get('region', 'OTHER')}]"
    return f"{mention} {region} • **{role_text}** {kc_text}{scythe}{freeze}"
...
async def setup(bot: commands.Bot):
    await bot.add_cog(SanguineCog(bot))**ToB Learner Resource Hub - made by Macflag**
